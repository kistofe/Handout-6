...
void j1Map::PropagateBFS()
 {
	...
	//pop the last item in frontier if it exists	
	iPoint Point = frontier.start->data;
	...
	//creating neighbor list to add all cords
	p2List<iPoint> neighbor;
	
	//add right neighbors
	neighbor.add({ Point.x + 1, Point.y });
	//add left neighbors
	neighbor.add({ Point.x - 1, Point.y });
	//add up neighbors
	neighbor.add({ Point.x, Point.y + 1 });
	//add down neighbors
	neighbor.add({ Point.x, Point.y - 1 });
	
	//We create an iterator variable to loop through all neighbors
	p2List_item<iPoint>* iterator = neighbor.start;
	for (iterator; iterator; iterator = iterator->next)
	{
		if (visited.find(iterator->data) == -1 && IsWalkable(iterator->data.x, iterator->data.y))//if the neighbor we find has not been visited, we add it to our lists
		{
			frontier.Push(iterator->data);
